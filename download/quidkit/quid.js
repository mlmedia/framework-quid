...
$(document).ready(function () {
	...
	$('.show_target').hide();
	$('.shown').show();
	$('.show_trigger').css('cursor', 'pointer'); // the show trigger should have the pointer hand so it "seems" like a link, even when it isn't
	$('.show_trigger').live('click', function (e) {
		var target_id = $(this).attr('rel');
		if ($('.show_target#' + target_id).hasClass('shown')) {
			$('.show_target#' + target_id).removeClass('shown').addClass('hidden');
		} else {
			$('.show_target#' + target_id).addClass('shown').removeClass('hidden');
		}
		$('.show_target#' + target_id).animate({
			opacity: 'toggle',
			height: 'toggle'
		}, 200);
		e.preventDefault();
		...
		function resizeBg() {
			if ((theWindow.width() / theWindow.height()) < aspectRatio) {
				if ($.browser.msie && parseInt($.browser.version, 10) < 8) {
					// hack for IE7 since it doesn't allow height 100%
					var nheight = theWindow.height();
					$bg.removeClass('full_width').css({
						'height': nheight
					});
				} else {
					$bg.removeClass('full_width').addClass('full_height');
				}
			} else {
				$bg.removeClass('full_height').addClass('full_width');
			}
		}
